...
import com.example.servicetesttool.Constants.Companion.AUTHORITY
...
class DataProvider : ContentProvider() {
    ...
    override fun insert(uri: Uri?, cv: ContentValues?): Uri? {
        ...
        val rId = db?.insert(DATUM_TABLE_NAME, Constants.TEXT, contentValues)
        if (rId != null) {
            if (rId > 0) {
    ...
    override fun query(
        ...
        queryBuilder.tables = DATUM_TABLE_NAME
        queryBuilder.projectionMap = projMap
        var s1 = s
        if (sUriMatcher.match(uri) != DATUM) {
